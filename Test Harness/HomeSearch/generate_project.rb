...
# Generates HomeSearch.xcodeproj.
#
# The project is generated rather than committed as a hand-edited pbxproj so the wiring to the local
# agent is explicit and reviewable: the Agent.xcodeproj subproject reference, the dependency on the
# Agent_iOS framework target, and the link + embed of NewRelic.framework are all visible here.
#
# Re-run after adding source files:
#
#     cd "Test Harness/HomeSearch" && ruby generate_project.rb
#
...
target = project.new_target(:application, 'HomeSearch', :ios, DEPLOYMENT_TARGET)
...
target.build_configurations.each do |config|
  ...
  settings['INFOPLIST_FILE']                  = 'HomeSearch/Info.plist'
  ...
  # The agent framework is built from source in this workspace; keep warnings from it out of the way
  # of the app's own diagnostics.
  settings['CLANG_ENABLE_MODULES']            = 'YES'
  settings['SWIFT_EMIT_LOC_STRINGS']          = 'YES'
end

# ── App sources, mirroring the on-disk folder structure ───────────────────────────────────────────
app_group = project.new_group('HomeSearch', 'HomeSearch')

%w[App Navigation Model Data Screens Components].each do |folder|
...
listings = resources_group.new_file('Data/listings.json')
target.add_resources([listings])
...
# The stub servers need an HTTP server. Swifter already lives in this repo for NRTestApp, so it is
# referenced from there: one copy, one place to update.
...
# Agent.xcodeproj is added as a subproject so the app links the framework built from THIS working
# tree. That is what makes the MobileViews API available at all: it does not exist in the published
# 7.7.6 XCFramework, only on this branch. Agent.xcodeproj carries MARKETING_VERSION = 7.7.6, so the
# framework the app links reports itself as 7.7.6.
# Adding a reference to a path ending in .xcodeproj makes xcodeproj build the whole subproject
# wiring for us: container item proxies, reference proxies for each product, a Products group, and
# the project_references entry on the root object.
...
# Agent.xcodeproj produces THREE products all named NewRelic.framework — iOS, tvOS and watchOS — so
# picking the proxy by name would be a coin flip between platforms. Match on the remote UUID of the
# Agent_iOS product instead, which is unambiguous.
...
scheme.save_as(PROJECT_PATH.to_s, 'HomeSearch', true)

puts "Generated #{PROJECT_PATH}"
puts "  target            HomeSearch (iOS #{DEPLOYMENT_TARGET})"
