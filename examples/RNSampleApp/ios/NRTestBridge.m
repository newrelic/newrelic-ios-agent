/*
 ...
 RCT_EXPORT_METHOD(recordStack:(NSString *_Nullable)errorName
     ...
     NSDictionary *dict = @{@"Name": errorName, @"Message": errorMessage, @"isFatal": isFatal, @"jsAppVersion": jsAppVersion, @"errorStack":
 error};
 ...
 RCT_EXPORT_METHOD(recordHandledException:(NSDictionary *_Nonnull)exceptionDictionary) {
     ...
     for (int i = 0; i < [stackFramesDict count]; ++i) {
         ...
         stackTraceElement[@"line"] = (currStackFrame[@"lineNumber"] && currStackFrame[@"lineNumber"] != [NSNull null]) ?
 currStackFrame[@"lineNumber"] : 0;
         stackTraceElement[@"method"] = (currStackFrame[@"methodName"] && currStackFrame[@"methodName"] != [NSNull null]) ?
 currStackFrame[@"methodName"] : @" ";
 ...
 RCT_EXPORT_METHOD(recordMetric:(NSString *)name
     ...
     if (value == -1) {
         ...
         if (valueUnit != nil && countUnit != nil) {
             [NewRelic recordMetricWithName:name category:category value:[NSNumber numberWithDouble:value] valueUnits:dict[valueUnit]
 countUnits:dict[countUnit]];
         } else {
 ...
 // MARK: - Agent Initialization (for testing configuration)
                                                                                                                                               
 RCT_EXPORT_METHOD(startAgent:(NSString *)appkey
                   agentVersion:(NSString *)agentVersion
                   reactNativeVersion:(NSString *)reactNativeVersion
                   customerConfiguration:(NSDictionary *)customerConfiguration) {
     ...
     if (useDefaultCollectorAddress && useDefaultCrashCollectorAddress) {
         ...
         NSString *collectorAddress = useDefaultCollectorAddress ? @"mobile-collector.newrelic.com" : [customerConfiguration
 objectForKey:@"collectorAddress"];
         NSString *crashCollectorAddress = useDefaultCrashCollectorAddress ? @"mobile-crash.newrelic.com" : [customerConfiguration
 objectForKey:@"crashCollectorAddress"];
