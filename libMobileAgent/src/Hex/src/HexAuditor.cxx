...
void HexAuditor::audit(uint8_t* buf) {
    ...
    auto agentDataBundle = com::newrelic::mobile::fbs::GetHexAgentDataBundle(buf);
    int indentations = 0;
    printStringWithIndentation("AgentDataBundle:\n[", indentations);
    indentations++;
    for (auto it = agentDataBundle->hexAgentData()->begin(); it != agentDataBundle->hexAgentData()->end(); it++) {
