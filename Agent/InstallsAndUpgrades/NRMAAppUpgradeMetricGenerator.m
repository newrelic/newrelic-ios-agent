...
- (void) didChangeAppVersion:(NSNotification*)notif {
    ...
    @synchronized(self) {
        ...
        // as a harvest listener. Once the harvest listener is fired this metric will be queue  from -onHarvestBefore.
        self.upgradeMetric = [[NRMAMetric alloc] initWithName:kNRMAAppUpgradeMetric
                                                        value:@1
                                                        scope:nil];

        if(self.analyticsController) {
            //this code path is unlikely to ever occur. The analytics engine will most likely never be initialized at the time of this execution.
            [self sendAppUpgradeToAnalytics:self.analyticsController];

            // Once the session attribute is recorded the analytics controller is no longer needed for this application lifecycle.
            self.analyticsController = nil;
        } else {
...
- (void) didInitializeAnalytics:(NSNotification*)notif {
    ...
    // Synchronized to ensure these operations are atomic in regards to the didChangeAppVersion notification.
    @synchronized(self) {
