...
public class SessionReplayManager: NSObject {
    ...
    public func start() {
        ...
        guard !isRunning() else {
            NRLogger.agentLogWarning("Session replay harvest timer attempting to start while already running.")
            return
        }

        NRLogger.agentLogDebug("Session replay harvest timer starting with a period of \(harvestPeriod) ms")
    ...
    public func stop() {
        sessionReplay.stop()
        guard isRunning() else {
            NRLogger.agentLogWarning("Session replay harvest timer attempting to stop when not running.")
            return
        }
        ...
        Task {
            ...
            if let jsonData = try? JSONEncoder().encode(container),
               let jsonString = String(data: jsonData, encoding: .utf8) {
                NRLogger.agentLogDebug(jsonString)
