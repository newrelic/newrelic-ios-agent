...
extension InspectableView: @preconcurrency Sequence where View: MultipleViewContent {
    ...
    public struct Iterator: @preconcurrency IteratorProtocol {
        ...
        private let group: LazyGroup<ViewInspectorContent>
        private let view: UnwrappedView
        
        init(_ group: LazyGroup<ViewInspectorContent>, view: UnwrappedView) {
            self.group = group
            self.view = view
        }
...
@available(iOS 13.0, macOS 10.15, tvOS 13.0, *)
extension ViewInspectorContent {
    
    static var absentView: ViewInspectorContent {
        return ViewInspectorContent(AbsentView())
    }
