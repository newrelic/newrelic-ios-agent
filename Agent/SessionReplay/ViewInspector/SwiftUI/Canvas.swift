...
extension ViewType.Canvas: SupplementaryChildren {
    static func supplementaryChildren(_ parent: UnwrappedView) throws -> LazyGroup<SupplementaryView> {
        ...
        return .init(count: 1) { _ in
            ...
            let content = try Inspector.unwrap(content: ViewInspectorContent(view, medium: medium))
...
public extension InspectableView where View == ViewType.Canvas {
    ...
    private func optionFlags() throws -> ViewType.Canvas.RasterizationOptions32 {
        ...
        if legacyOptions.contains(ViewType.Canvas.RasterizationOptions8.opaque) {
            options.insert(ViewType.Canvas.RasterizationOptions32.opaque)
        }
        if legacyOptions.contains(ViewType.Canvas.RasterizationOptions8.rendersAsynchronously) {
            options.insert(ViewType.Canvas.RasterizationOptions32.rendersAsynchronously)
        }
        return options
    }
}
