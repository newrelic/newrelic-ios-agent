...
internal extension ViewInspectorContent {
    func unwrappedModifiedContent() throws -> ViewInspectorContent {
        let view = try Inspector.attribute(label: "content", value: self.view)
        var medium: ViewInspectorContent.Medium
        if let modifier = self.view as? EnvironmentModifier,
           modifier.qualifiesAsEnvironmentModifier() {
            ...
            if let modifier = (self.view as? ModifierNameProvider)?.customModifier,
               let modifierBodyContent = try? ViewInspectorContent(modifier, medium: self.medium).extractCustomView(),
