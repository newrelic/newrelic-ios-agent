...
internal extension ViewInspectorContent {
    ...
    func toolbar(parent: UnwrappedView, index: Int?) throws -> InspectableView<ViewType.Toolbar> {
        ...
        let content = try Inspector.unwrap(content: ViewInspectorContent(root, medium: medium))
...
public extension InspectableView where View == ViewType.Toolbar {
    ...
    func item(_ index: Int = 0) throws -> InspectableView<ViewType.Toolbar.Item> {
        ...
        return try .init(ViewInspectorContent(element, medium: content.medium), parent: self, index: index)
    }
    ...
    func element(_ index: Int) throws -> Any {
        do {
