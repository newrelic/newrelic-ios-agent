...
internal extension ViewInspectorContent {
    ...
    func overlay(parent: UnwrappedView, api: [ViewType.Overlay.API],
        ...
        let content = try Inspector.unwrap(content: ViewInspectorContent(rootView, medium: medium))
...
internal extension ViewType.Overlay.API {
    ...
    func verifySignature(content: Any, modifier: Any, hasMultipleOverlays: Bool) throws {
        ...
        case .overlayStyle, .backgroundStyle:
            break
        case .border:
            let stroke = try? InspectableView<ViewType.Shape>(ViewInspectorContent(content), parent: nil, index: nil).strokeStyle()
            if stroke == nil {
                try reportFailure()
            }
        case .overlay:
