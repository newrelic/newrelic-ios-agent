...
public extension InspectableView {
    ...
    func accessibilityLabel() throws -> InspectableView<ViewType.Text> {
        if #available(iOS 15.0, macOS 12.0, tvOS 15.0, watchOS 8.0, *),
           ...
           let label = try? InspectableView<ViewType.Text>(ViewInspectorContent(text), parent: nil)
            .attributes().accessibilityLabel() {
            let medium = content.medium.resettingViewModifiers()
            return try .init(try Inspector.unwrap(content: ViewInspectorContent(label, medium: medium)), parent: self)
        }
        let text: Text
        ...
        let medium = content.medium.resettingViewModifiers()
        return try .init(try Inspector.unwrap(content: ViewInspectorContent(text, medium: medium)), parent: self)
    }
...
private extension AccessibilityActionKind {
    func name() throws -> InspectableView<ViewType.Text> {
        ...
        return try .init(ViewInspectorContent(view), parent: nil, index: nil)
    }
