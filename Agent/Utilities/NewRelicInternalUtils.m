...
#define kCOLLECTOR_HEX_URL             @"/mobile/f"
...
+ (BOOL) isInteger:(NSNumber*)number {
    ...
    switch ([number objCType][0]) {
        ...
        case 'S':
        case 'L':
        // integer types; casing all unsigned to signed
        case 'Q':
            return YES;
        default:
            return NO;
...
{
    return [self collectorHostURL:(NSString*)kCOLLECTOR_HEX_URL];
}
...
+ (NSString*) carrierName {
    ...
    // after that all callers will succeed fast for kNRMACarrierNameCacheLifetime milliseconds, with only the overhead of the locking code.

    // We double check for the same thing before and after synchronization.
    // We might encounter unnecessary sync blockage here if we do the check beforehand and we will prevent multiple checks.
...
+ (NSString*) normalizedStringFromString:(NSString*)url {
    ...
    // Correct an issue in the NSURL validator, which will allow a single slash after the protocol.
...
#if TARGET_OS_TV
    // tvOS doesn't support the documents dir. The cache dir is the next best thing. (It is used by the crash reporter, which means it should work O.K.)
